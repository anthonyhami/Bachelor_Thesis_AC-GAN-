...
def define_discriminator(in_shape=(976,1), n_classes=39):
    ...
    for i in range(int(num_of_layers_disc)):
        ...
        fe=(Dense(list_of_neurons_disc[i],kernel_initializer=init ,activation="sigmoid"))(fe)
...
def define_generator(latent_dim, n_classes=39, in_shape=976):
    ...
    for i in range(int(num_of_layers_gen)):
        # Add one hidden layer
        merge=(Dense(list_of_neurons_gen[i],kernel_initializer=init, activation="sigmoid"))(merge)
        #print ("merge ========= ", merge)
...
def train(g_model, d_model, gan_model, dataset, latent_dim, n_epochs=500, n_batch=num_of_batch):
